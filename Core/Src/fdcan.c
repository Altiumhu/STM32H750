    ...
    if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != 0) {
        ...
        if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK) {
            if (RxHeader.IdType == FDCAN_EXTENDED_ID) {
                ...
                uint8_t fc = (id >> ID_FC_POS) & 0x0F;
                uint16_t did = (id >> ID_DID_POS) & 0x1FFF;
                uint16_t sub = id & 0x0FFF;
                
                // 处理大数据传输
                if (fc == FC_DATA_TRANSFER && RxHeader.DataLength >> 16 == sizeof(LargeDataFrame)) {
                    ...
                    // 其他功能码处理
                    ProcessCANMessage(fc, did, sub, RxData, RxHeader.DataLength >> 16);
                }
            }
        }
    }
