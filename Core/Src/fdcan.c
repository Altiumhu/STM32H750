...
void MX_FDCAN1_Init(void)
{
  ...
  /* USER CODE BEGIN FDCAN1_Init 2 */
    ...
    fdcan_filterconfig.FilterID2 = 0x0;                            /* 过滤ID2：配置为传统位过滤，ID2是11位掩码 
...
void MX_FDCAN2_Init(void)
{
  ...
  /* USER CODE BEGIN FDCAN2_Init 2 */
    ...
    fdcan_filterconfig.FilterIndex = 0;                              /* 滤波器索引 */
    fdcan_filterconfig.FilterType = FDCAN_FILTER_MASK;               /* 滤波器类型：传统位过滤 */
    fdcan_filterconfig.FilterConfig = FDCAN_FILTER_TO_RXFIFO1;       /* 过滤配置：当过滤匹配以后存储在Rx FIFO0中 */
    fdcan_filterconfig.FilterID1 = 0x123;                            /* 过滤ID1：11位标准ID */
    fdcan_filterconfig.FilterID2 = 0x0;                            /* 过滤ID2：配置为传统位过滤，ID2是11位掩码 
                                                                      * 这里表示过滤接收和FilterID1完全一样的消息ID 
                                                                      */
...
uint8_t fdcan_send_msg(uint8_t *msg, uint32_t len)
{
    ...
    g_fdcanx_txheade.IdType = FDCAN_EXTENDED_ID;                     /* 11位标准ID */
...
{
    /* 从RX FIFO0检索接收的消息 */
    if (HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO0, &g_fdcanx_rxheade, buf) != HAL_OK)  /* 接收数据 */
    {
        return 0;
    }
