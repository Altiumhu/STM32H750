...
void GetADC_Driver_Result(void)
{
    ...
    static uint16_t Samp_ChnumTimer = 0;  // 通道切换计时器
    ...
    switch (CD4052_Chnum)
    {
        ...
        case 1:
            ...
            g_Channelinfo[5].voltage_ADC  = adc1Result[1];   // 通道6 电压
            g_Channelinfo[9].voltage_ADC  = adc1Result[2];   // 通道10 电压
        ...
        case 2:
            ...
            g_Channelinfo[9].voltage_ADC  = adc1Result[2];   // 通道11 电压
            g_Channelinfo[14].voltage_ADC = adc1Result[3];  // 通道15 电压

            /* 电流采样 */
            g_Channelinfo[2].current_ADC  = adc1Result[4];   // 通道3 电流
            g_Channelinfo[6].current_ADC  = adc1Result[5];   // 通道7 电流
            g_Channelinfo[9].current_ADC  = adc1Result[6];   // 通道11 电流
            g_Channelinfo[14].current_ADC = adc1Result[7];  // 通道15 电流
            ...
            {
                Set_Sample_Channel_VPortGPIO(AD_V_CAP_EN);
                g_Channelinfo[1].Cap_voltage_ADC = adc1Result[12]; // 通道3 电容电压
                g_Channelinfo[5].Cap_voltage_ADC = adc1Result[13]; // 通道7 电容电压
            }
            else if (g_Channelinfo[0].workMode == POWER_FAULT || g_Channelinfo[0].workMode == POWER_RUN_CHARGE)
            {
                Set_Sample_Channel_VPortGPIO(AD_V_PORT);
                g_Channelinfo[1].voltage_port_ADC = adc1Result[12]; // 通道3 端口电压
                g_Channelinfo[5].voltage_port_ADC = adc1Result[13]; // 通道7 端口电压
            }
            break;
...
void ADC_Convert_Vaule(void)
{
     ...
     * 16位ADC分辨率: 3.3V / 65535 = 5.03548E-05 V/LSB
     */

    /* 通道1电压: 分压系数2.2 */
    g_Channelinfo[0].voltage = adc_values[0] * 5.03548E-05f * 2.2f;
