...
#define IIC_ADDR	0x00			//对应SMEC88中地址

//---------------------------------------------------------
//函数名: 获取加密芯片SMEC88的UID
//参数说明：
//			pUID - UID存放指针
//返回值说明：
//			0 - 成功
...
//函数名: SHA1认证
//参数说明：
//			pbSha1Key  	- SHA1认证密钥，实际为做SHA1运算的前置数据, 在主控芯片及SMEC88内部存在, 不用在I2C线路上传输
...
//			0 - 成功
//			1 - 失败					   
//说明:主控芯片先对pbSha1Key做HASH(摘要)运算,然后再对pbRandom继续做HASH运算. 将其结果与SMEC88返回值作比较. 
//      pbSha1Key分别放于主控芯片及SMEC88中,无需在线路上传输.这样如果主控芯片与加密芯片的pbSha1Key不一致, 则无法验证成功
//---------------------------------------------------------
...
//			0 - 成功
//			1 - 失败					   
//说明:主控芯片向SMEC88发送8字节随机数, SMEC88将随机数进行3DES加密后送给主控芯片, 由主控芯片判断回送数据的合法性
//---------------------------------------------------------
...
//			0 - 成功
//			1 - 失败					   
//说明:主控芯片先获取SMEC88的8字节随机数, 然后用bKey对随机数做3DES加密，再将密文送给SMEC88
//---------------------------------------------------------
...
//			0 - 成功
//			1 - 失败					   
//说明: 将 MCU 中的一部分关键代码，放入加密芯片中运行，当需要用到SMEC88中的算法时，
//      由MCU 向SMEC88 发送指令，SMEC88 根据指令，在内部运行，返回结果给MCU。
//---------------------------------------------------------
...
unsigned char SMEC_CircleAlgCrypt(unsigned char *pbSeesionKey, unsigned char *pbInput, unsigned char bInputLen, unsigned char *pbOutput, unsigned char *pbOutputLen)
{
	...
	encrypt(bTmp, pbSeesionKey, &bBuf[4]);				   //将端口数据加密,传给SMEC88,让其运算
...
//      过程密钥的产生:  pbMKey 对 (pbRandom ^ SMEC88随机数)做3DES加密运算, 所得结果为 pbSeesionKey(8字节)
//---------------------------------------------------------
unsigned char SMEC_GenSessionKey(unsigned char *pbMKey, unsigned char *pbRandom, unsigned char *pbSeesionKey)
{
	...
	for(i = 0; i < 8; i ++)
	{
	 	bTmp[i] = pbRandom[i] ^ bBuf[2 + i]; 				//将SMEC88的随机数与主控芯片随机数做异或运算
	}
...
//  PA端口数据2字节, 用过程密钥加密后,送给SMEC88,由SMEC88解密后取反,再由过程密钥加密回送给主控芯片.这样就构造出一个算法
//  如PA = 0x0000, 用过程密钥加密送给SMEC88, SMEC88解密后得到0x0000, 取反后为0xFFFF, 再用过程密钥加密给主控芯片. 主控芯片解密后得到0xFFFF
...
unsigned char SMEC_GpioAlg(unsigned char *pbSeesionKey, unsigned char *pbGpioInput, unsigned char bGpioDataLen, unsigned char *pbGpioOutput)
{
	...
	encrypt(bTmp, pbSeesionKey, &bBuf[4]);				   //将端口数据加密,传给SMEC88,让其"运算"
