...
FDCAN_HandleTypeDef       g_fdcanx_handle;             /* FDCANx句柄 */
FDCAN_TxHeaderTypeDef     g_fdcanx_txheade;            /* 发送消息 */
FDCAN_RxHeaderTypeDef     g_fdcanx_rxheade;            /* 接收消息 */

FDCAN_HandleTypeDef       g_fdcan3_handle;             /* FDCANx句柄 */
FDCAN_TxHeaderTypeDef     g_fdcan3_txheade;            /* 发送消息 */
FDCAN_RxHeaderTypeDef     g_fdcan3_rxheade;            /* 接收消息 */
/**
 * @brief       FDCAN初始化
 ...
 *              配置FDCAN的时钟源为PLL1Q, FDCAN输入时钟频率 fdcan_ker_ck = pll1_q_ck = 40Mhz
 *              波特率 = fdcan_ker_ck / ((ntseg1 + ntseg2 + 1) * presc);
 *              我们设置 fdcan_init(1, 8, 31, 8, mode), 则CAN波特率为:
 *              40M / ((31 + 8 + 1) * 1) = 1Mbps
 ...
 * @retval      0, 初始化成功; 其他, 初始化失败;
 */
uint8_t fdcan3_init(uint16_t presc, uint8_t tsjw, uint16_t ntseg1, uint8_t ntseg2, uint32_t mode) 
{
    FDCAN_FilterTypeDef fdcan_filterconfig;

    HAL_FDCAN_DeInit(&g_fdcan3_handle);                              /* 先清除以前的设置 */
    g_fdcan3_handle.Instance = FDCAN3; 
    g_fdcan3_handle.Init.FrameFormat = FDCAN_FRAME_CLASSIC;          /* 传统模式 */
    g_fdcan3_handle.Init.Mode = mode;                                /* 操作模式设置  */
    g_fdcan3_handle.Init.AutoRetransmission = DISABLE;               /* 关闭自动重传！传统模式下一定要关闭！！！  */
    g_fdcan3_handle.Init.TransmitPause = DISABLE;                    /* 关闭传输暂停 */
    g_fdcan3_handle.Init.ProtocolException = DISABLE;                /* 关闭协议异常处理 */
  
    g_fdcan3_handle.Init.NominalPrescaler = presc;                   /* 设置比特率分频系数 */
    g_fdcan3_handle.Init.NominalSyncJumpWidth = tsjw;                /* 设置重新同步跳跃宽度 */
    g_fdcan3_handle.Init.NominalTimeSeg1 = ntseg1;                   /* 设置位时间段1 */
    g_fdcan3_handle.Init.NominalTimeSeg2 = ntseg2;                   /* 设置位时间段2 */
  
    g_fdcan3_handle.Init.MessageRAMOffset = 0;                       /* 消息RAM偏移 */
    g_fdcan3_handle.Init.StdFiltersNbr = 1;                          /* 标准消息ID过滤器数量 */
    g_fdcan3_handle.Init.ExtFiltersNbr = 0;                          /* 扩展消息ID过滤器数量 */
    g_fdcan3_handle.Init.RxFifo0ElmtsNbr = 1;                        /* 接收FIFO0元素数量 */
    g_fdcan3_handle.Init.RxFifo0ElmtSize = FDCAN_DATA_BYTES_8;       /* 接收FIFO0元素数据字段大小：8字节 */
    g_fdcan3_handle.Init.RxBuffersNbr = 0;                           /* 专用接收缓冲区元素数量 */
    g_fdcan3_handle.Init.TxEventsNbr = 0;                            /* 发送事件FIFO元素数量 */
    g_fdcan3_handle.Init.TxBuffersNbr = 0;                           /* 专用发送缓冲区数量 */
    g_fdcan3_handle.Init.TxFifoQueueElmtsNbr = 1;                    /* 用于发送FIFO/队列的发送缓冲区数量 */
    g_fdcan3_handle.Init.TxFifoQueueMode = FDCAN_TX_FIFO_OPERATION;  /* 发送FIFO模式 */
    g_fdcan3_handle.Init.TxElmtSize = FDCAN_DATA_BYTES_8;            /* 发送缓冲区数据字段大小：8字节 */

    if (HAL_FDCAN_Init(&g_fdcan3_handle) != HAL_OK)                  /* 初始化FDCAN */
    {
        return 1;   
    }
    ...
    if (HAL_FDCAN_ConfigFilter(&g_fdcan3_handle, &fdcan_filterconfig) != HAL_OK) 
    {
        return 2;                                            
    }

    /* 配置全局过滤器，拒收所有不匹配的帧 */
    HAL_FDCAN_ConfigGlobalFilter(&g_fdcan3_handle, FDCAN_REJECT, FDCAN_REJECT, FDCAN_FILTER_REMOTE, FDCAN_FILTER_REMOTE);

    /* 开启FDCAN */
    if (HAL_FDCAN_Start(&g_fdcan3_handle) != HAL_OK)
    {
        return 3;
    }  

    /* 使能接收FIFO 0新消息中断 */
    HAL_FDCAN_ActivateNotification(&g_fdcan3_handle, FDCAN_IT_RX_FIFO0_NEW_MESSAGE, 0);
...
//        __HAL_RCC_FDCAN_CLK_ENABLE();                         /* 使能FDCAN时钟 */
//        FDCAN3_RX_GPIO_CLK_ENABLE();                           /* FDCAN_RX引脚时钟使能 */
//        FDCAN3_TX_GPIO_CLK_ENABLE();                           /* FDCAN_TX引脚时钟使能 */
//        
//        gpio_init_struct.Pin = FDCAN3_RX_GPIO_PIN;             /* FDCAN_RX引脚 */
//        gpio_init_struct.Mode = GPIO_MODE_AF_PP;              /* 复用推挽输出 */
//        gpio_init_struct.Pull = GPIO_PULLUP;                  /* 上拉 */
//        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;        /* 快速 */
//        gpio_init_struct.Alternate = FDCAN_RX_GPIO_AF;        /* 复用为FDCAN1 */
//        HAL_GPIO_Init(FDCAN3_RX_GPIO_PORT, &gpio_init_struct); /* 初始化FDCAN_RX引脚 */

//        gpio_init_struct.Pin = FDCAN3_TX_GPIO_PIN;             /* FDCAN_TX引脚 */
//        gpio_init_struct.Alternate = FDCAN_TX_GPIO_AF;        /* 复用为FDCAN1 */
//        HAL_GPIO_Init(FDCAN_TX_GPIO_PORT, &gpio_init_struct); /* 初始化FDCAN_TX引脚 */

//#if FDCAN3_RX0_INT_ENABLE                                     /* 如果使能了FDCAN1接收RX0中断 */
//        HAL_NVIC_EnableIRQ(FDCAN1_IT0_IRQn);                  /* 使能FDCAN1中断线0中断 */
//        HAL_NVIC_SetPriority(FDCAN1_IT0_IRQn, 1, 2);          /* 抢占优先级1，子优先级2 */
//#endif
//    }
...
{
    g_fdcan3_txheade.Identifier = 0x123;                             /* 帧ID */
    g_fdcan3_txheade.IdType = FDCAN_STANDARD_ID;                     /* 11位标准ID */
    g_fdcan3_txheade.TxFrameType = FDCAN_DATA_FRAME;                 /* 发送数据帧 */
    g_fdcan3_txheade.DataLength = len;                               /* 数据长度 */
    g_fdcan3_txheade.ErrorStateIndicator = FDCAN_ESI_ACTIVE;         /* 传输节点显性错误 */
    g_fdcan3_txheade.BitRateSwitch = FDCAN_BRS_OFF;                  /* 关闭速率切换 */
    g_fdcan3_txheade.FDFormat = FDCAN_CLASSIC_CAN;                   /* 标准帧格式 */
    g_fdcan3_txheade.TxEventFifoControl = FDCAN_NO_TX_EVENTS;        /* 不存储发送事件 */
    g_fdcan3_txheade.MessageMarker = 0;                              /* 消息掩码 */ 

    /* 添加消息到发送FIFO并添加相应的发送请求 */
    if (HAL_FDCAN_AddMessageToTxFifoQ(&g_fdcan3_handle, &g_fdcan3_txheade, msg) != HAL_OK)  /* 发送消息 */
    {
        return 1;
    }
 ...
 *              其他, 接收到的数据长度;
 */
uint8_t fdcan3_receive_msg(uint8_t *buf)
{
    /* 从RX FIFO0检索接收的消息 */
    if (HAL_FDCAN_GetRxMessage(&g_fdcan3_handle, FDCAN_RX_FIFO0, &g_fdcan3_rxheade, buf) != HAL_OK)  /* 接收数据 */
    {
        return 0;
    }
