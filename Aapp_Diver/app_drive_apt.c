...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIMX_PWM)                                 /* PWM输出定时器 */
    {
        ...
        gpio_init_struct.Pin = TIMX_PWM_CHY_GPIO_PIN|TIMX_PWM_CH2_GPIO_PIN;               /* 通道Y的GPIO口 */
...
//			  gpio_init_struct.Pin = TIMX_PWM_CH2_GPIO_PIN;               /* 通道Y的GPIO口 */
//        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                    /* 复用推挽输出 */
//        gpio_init_struct.Pull = GPIO_PULLUP;                        /* 上拉 */
//        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;              /* 快速 */
//        gpio_init_struct.Alternate = TIMX_PWM_CH2_GPIO_AF;          /* 定时器TIMX通道Y的GPIO口复用功能选择 */
//        HAL_GPIO_Init(TIMX_PWM_CH2_GPIO_PORT, &gpio_init_struct);   /* 初始化通道Y引脚 */
    }
}
...
//        /* 修改比较值控制占空比 */
        __HAL_TIM_SET_COMPARE(&g_timx_pwm_chy_handle, TIMX_PWM_CHY, 150);
      //  __HAL_TIM_SET_COMPARE(&g_timx_pwm_chy_handle, TIM3_PWM_CH2, 150);
}
